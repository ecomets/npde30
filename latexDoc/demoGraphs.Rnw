...
fromLib<-FALSE # Toggle to compile from (installed) library
if(fromLib) {
  library(npde)
} else {
  chezRomain<-FALSE
  ...
  if(chezRomain) {
    npdeDir<-"D:/RECHERCHES/INSERM_IAME/[R] - Package - NPDE/npde30-master/npde30-master"
    docDir<-file.path(npdeDir,"documentation")
  } else {
  ...
  #setwd(workDir)
  npdeDir<-file.path(workDir,"PackageNpdeBetaVersion") # current directory for R functions (R subfolder)
  dataDir<-file.path(npdeDir,"data") # data files, including remifentanil
  docFigs<-file.path(docDir,"figs")
  docTex<-file.path(docDir,"doc3.0")
  saveDocPlots<-FALSE # whether to save the plots 
  
  # Loading library functions
  # Classes
  source(file.path(npdeDir,"R","aaa_generics.R"))
  source(file.path(npdeDir,"R","NpdeData.R"))
  source(file.path(npdeDir,"R","NpdeRes.R"))
  source(file.path(npdeDir,"R","npdeControl.R"))
  source(file.path(npdeDir,"R","NpdeSimData.R"))
  source(file.path(npdeDir,"R","NpdeObject.R"))
  
  # Create, show, print methods for objects
  source(file.path(npdeDir,"R","NpdeData-methods.R"))
  source(file.path(npdeDir,"R","NpdeRes-methods.R"))
  source(file.path(npdeDir,"R","NpdeObject-methods.R"))
  
  # Package methods
  source(file.path(npdeDir,"R","compute_distribution.R"))
  source(file.path(npdeDir,"R","compute_npde.R"))
  source(file.path(npdeDir,"R","compute_pd.R"))
  source(file.path(npdeDir,"R","compute_ploq.R"))
  source(file.path(npdeDir,"R","npde.R"))
  source(file.path(npdeDir,"R","mainNpde.R"))
  
  # Plot methods
  source(file.path(npdeDir,"R","plotNpde-methods.R"))
  source(file.path(npdeDir,"R","plotNpde-plotFunctions.R"))
  source(file.path(npdeDir,"R","plotNpde-unitFunctionsAux.R"))
  source(file.path(npdeDir,"R","plotNpde-unitFunctionsPI.R"))
  source(file.path(npdeDir,"R","plotNpde-meanProf.R"))
  source(file.path(npdeDir,"R","plotNpde-unitFunctionsRef.R"))
}
@
...
if(fromLib) {
  data(warfarinPK)
  data(warfarinSimBase)
  data(warfarinSimCov)
  ybase<-autonpde(namobs=warfarinPK, namsim=warfarinSimBase, iid=1, ix=2, iy=4, icov=c(6:8), units=list(x="hr", y="mg/L", cov=c("kg","-","yr")))
  cat("      npde for the model with covariates\n")
  ycov<-autonpde(namobs=warfarinPK, namsim=warfarinSimCov, iid=1, ix=2, iy=4, icov=c(6:8), units=list(x="hr", y="mg/L", cov=c("kg","-","yr")))
  } else {
