% Output
% dvips userguide_3.0
% ps2pdf userguide_3.0.ps;mv userguide_3.0.pdf userguide_npde3.0.pdf
...
\hypersetup{pdftitle="Userguide for npde 3.0",pdfauthor="Emmanuelle Comets",
...
{\Large \bfseries User guide for npde 3.0}

{\Large \bfseries January 2021}

\bigskip 

{\Large \itshape \bfseries Emmanuelle Comets}

{\large \itshape \bfseries Contributors to the npde package: Karl Brendel, Marc Cerou,  Romain Leroux, Thi Huyen Tram Nguyen, France Mentr\'e}
...
\hskip 18pt  This is the User's guide for the add-on package {\sf npde} (version 3.0, January 2021) for the {\sf R} language. It is distributed in the \texttt{inst} directory of the package. Version 3.0 includes a complete overhaul of the graphs, which have been moved to the {\sf ggplot2} philosophy~\cite{ggplot2} by Romain Leroux, as well as the addition of reference plots.

{\sf npde} computes normalised prediction distribution errors ($npde$), a metric to evaluate nonlinear mixed-effect models such as those used in population pharmacokinetic or pharmacodynamic studies. Prediction distribution errors were developed under the name prediction discrepancies by Mentr\'e and Escolano~\cite{MentrePDE}. Brendel et al~\cite{BrendelPDE} proposed an improved version taking into account repeated observations within each subject, which were called prediction distribution errors. Both prediction discrepancies and prediction distribution errors are usually transformed to a normal distribution for graphical diagnostics.
