...
# Where the package was compiled on my machine
compileDir<-"/home/eco/work/npde/compileNpde"

# Toggles saving plots for the documentation
saveDocPlots<-FALSE
...
install.packages(pkgs=file.path(compileDir,"npde_3.3.tar.gz"),repos=NULL)
library(npde)
...
wcov devrait marcher (avec les covariables données en numéro de colonne et pas avec leurs noms)

```{r warfarinRun, include=FALSE,echo=FALSE,error=FALSE, warning=FALSE,message=FALSE}
data(warfarin)
simwarfarinBase<-read.table(file.path(fullDatDir, "simwarfarinBase.tab"), header=T)
simwarfarinCov<-read.table(file.path(fullDatDir, "simwarfarinCov.tab"), header=T)
...
wcov2<-autonpde(namobs=warfarin,namsim=simwarfarinCov, iid=1,ix=2,iy=4,icov=c("amt" ,"wt" , "sex","age"),namsav="warfCov", units=list(x="hr",y="ug/L", covariates=c("mg","kg","-","yr")))

# Warfarin with MDV
warfmdv<-read.table(file.path(datDir, "warfarin.tab"), header=T)
...
- **Change**
  - npd now default for diagnostic graphs (npde still default for tests)
  - **TODO** change output for test function
...
- **BUGS Romain TODOO**
  ...
  - **solved**
    ...
    -  axes in the histogram plot => add
...
# plot by default
plot(theofit1)
plot(theofit1, main="Gof plots for theophylline")
plot(theofit1, which="pd")
plot(theofit1, which="npde")
plot(theofit1, which="faux") # switches to npd automatically
...
- **Note**   - ok mais pour le moment que pour : hist, qqplot, x.scatter, pred.scatter, vpc, ecdf, data
  - le pipeline est le suivant : fonction auxliaire plot --> fonction plot --> plot dans plotNpde-methods
    ...
    - **Eco** modifié car le code plantait, toujours ce mélange de [[1]]
...
grid.arrange(grobs=list(plot.x.scatter.yvir50, plot.pred.scatter.yvir50), nrow=1, ncol=2)

plot.vpc.yvir50 = plot(yvir50,plot.type="vpc")
plot.yvir50.omit = plot(yvir50.omit,plot.type="vpc")
namfile<-file.path(docFigs,"doc_sidebysideVPC.eps")
if(saveDocPlots) cairo_ps(file = namfile, onefile = TRUE, fallback_resolution = 600, height=8.27, width=11.69)
grid.arrange(grobs=list(plot.vpc.yvir50,plot.yvir50.omit), nrow=1, ncol=2)
if(saveDocPlots) dev.off()

```
- **Note**
- Pour modifier les graphes directement avec un template ggplot. Le theme d'un plot ggplot est accessible via le slot theme et layers, exemple : *plot.vpc.yvir50$theme et plot.vpc.yvir50$layers*.On peut ainsi définir un template ggplot par defaut pour chaque plot et créer une fonction qui va directement le modifier en  fonctions des options utilisateurs. Comme *plot.vpc.yvir50$theme* et *plot.vpc.yvir50$layers* sont des listes, avec modifyList les slots sont facilement accessibles et modifiables. Dans les fonctions plots pourraient être ainsi appelées un theme tels que *theme_plot_vpc(), theme_plot_scatter(), etc.*
