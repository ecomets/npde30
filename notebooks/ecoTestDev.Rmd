...
- **BUGS Romain TODOO**
  ...
  - dans plotNpde-auxScatterPlot: 1er bin avec valeur minimal de la borne et dernier bin avec valeur maximal de la borne. 1er et dernier le plot de s'arrête pas au milieu.
...
- **Problems**  \textcolor{red}{(à résoudre Romain)}
- **Note** pour cette version ça fait beaucoup à modifier, à commencer par plotNpde-methods puis npde.plot.data où lee slota data est à modifier. Il faudrait distinguer le plot par défaut et *plot(yivr50@data)* sans arguments supplémentaires. 
  ...
  - **pour cette version trop de chose à modifier pour ça**
...
plot(wcov, covsplit=TRUE, which.cov=c("sex","wt"),main=c("waffle plot : Sex","waffle plot : Wt"))
...
- **TODO**
  ...
  - **note** 
  - Maintenant les foncions graphiques renvoient objet=NULL pour les plots, avant non. Peut-être faire remonter le return des objets
  ggplots jusque dans la fonction "plot" générale?
...
p1 = plot(yvir50,plot.type="x.scatter")
p2 = plot(yvir50,plot.type="pred.scatter")
# p1 = NULL, p2 = NULL
try(do.call(grid.arrange, c(p1, p2, list( nrow=1, ncol=2))))
